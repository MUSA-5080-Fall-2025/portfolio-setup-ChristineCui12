...
join_dedup <- join_result %>%
  st_drop_geometry() %>%
  distinct(GEOID, sale_id, .keep_all = TRUE)

opa_summary <- join_dedup %>%
  group_by(GEOID) %>%
  summarise(
    past_count = sum(!is.na(sale_id)),
...
opa_census <- opa_census %>%
  left_join(opa_summary, by = "GEOID")
```
...
opa_census <- opa_census_m %>%
  left_join(poi_summary, by = "GEOID")
