...
neighbors <- pa_counties %>%
  st_filter(allegheny, .predicate = st_touches) #st:doing sth spatial
...
allegheny_center <- st_centroid(allegheny)  # polygon-center
nearby <- pa_counties %>%
...
tracts_with_counties <- census_tracts %>%
  st_join(pa_counties)   #不用想right left
...
pa_counties <- pa_counties %>%
  mutate(
    area_sqkm = as.numeric(st_area(.)) / 1000000  # Convert to sq km  (st_area(.)点的作用：看slide
  )

# Population density (need to join census data) （有问题）
pa_counties <- pa_counties %>%
...
# Distance from each county to a specific point（有问题）
