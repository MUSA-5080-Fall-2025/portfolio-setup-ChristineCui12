...
- Rows: 50,000
- Columns: 7
- Variable types: numeric<dbl>, character<chr>
- Problematic names: **??????**
...
**Your answer:** **Tibble** demonstrates the first 10 rows, and shows the data type (e.g., <dbl> for double/numeric, <chr> for character) underneath each column name. Besides, it provides a concise, tidy preview that prevents large datasets from overwhelming the console. **Data Frame** attempts to display all rows and all columns, and it does not show the data type for each column when printed.
...
**Your answer:** Because year is a single word containing only letters, numbers, and underscores. SQL can understand it unambiguously without any special quoting. Year of manufacture contains spaces. To SQL, a space typically separates different parts of a command. The backticks tell the SQL parser: "Treat everything inside these quotes as a single unit, a single column name, even if it has spaces or special characters."
