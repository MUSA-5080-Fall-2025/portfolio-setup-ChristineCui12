...
# 下载费城县的 tract-level 数据 (geometry = TRUE 会包含地理边界)
...
philly_tracts <- philly_tracts %>%
  ...
  st_as_sf() %>%
  st_transform(3857) %>%  # 投影到适合空间计算的坐标（如 Web Mercator）
...
green_access_map <- ggplot(philly_tracts_proj) +
  ...
  # 树木位置（抽样显示以避免过度拥挤）
  geom_sf(
    data = trees_proj %>% sample_n(min(5000, nrow(trees_proj))),  # 限制显示数量
    color = "#238b45",
    size = 0.1,
    alpha = 0.3
  ) +
...
The statistical analysis showed a positive correlation between the percentage of nonwhite residents and distance to the park (r≈0.288), and a negative correlation between median income and park distance (r≈−0.197). This means that census tracts with a higher proportion of nonwhite residents or lower incomes tend to be farther away from the nearest park.  
